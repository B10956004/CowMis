  ...
  <div name="content" style="width:100%; height: 100vh ;   padding:1.5rem  ;   ">
    <div class="tab-content">
      <ul class="nav nav-tabs" role="tablist">
        <li class="nav-item">
          <a href="#growthInformation" class="nav-link active" data-toggle="tab">生長資訊</a>
        </li>
        <li class="nav-item">
          <a href="#addGrowthInformation" class="nav-link" data-toggle="tab">新增資料</a>
        </li>
      </ul>
      <div class="tab-pane active bg-white shadow-sm p-4" id="growthInformation">
        <div class="row">
          ...
          <div id="cow_table" style="text-align:center;">
            ...
            <table id="rule" class="table table-hover">
              ...
              <tbody>
                ...
                $query = "SELECT * FROM parturition_history WHERE isDel=0 ";
                $result = mysqli_query($db_link, $query);
                    ...
                    <script>
                      $(\"#linkDel_$i\").click(function(){
                        var yesDel = confirm(\"你確定要刪除\"+'$id'+\"這筆資料嗎？，刪除後不可復原。\");
                          if (yesDel) {
                            $.post(\"./growthInformation_Delete.php\",{ Del: 1,postSn:$sn },function(result){
                              location.reload();
                              });
                          }
                        });
      ...
      <script>
        $(document).on('click', '.view_data', function() {
          ...
          $.ajax({
            url: "growthInformation_Revise.php",
            method: "GET",
            data: {
              GetSn: GetSn
            },
      ...
      <!-- 切換頁 新增 -->
      <div class="tab-pane fade bg-white shadow-sm p-4" id="addGrowthInformation" style="border-radius:5px;">
        <form action="growthInformation_Insert.php" method="post">
          <div class="row">
            <div class="col-md-3 p-2">
              <input type="text" class="form-control " placeholder="請輸入編號" name="id" required>
            </div>
            <div class="col-md-3 p-2">
              <input type="date" class="form-control " placeholder="請輸入出生日期" name="date" required>
            </div>
            <div class="col-md-3 p-2">
              <input type="text" class="form-control " placeholder="請輸入胎次" name="birthParity" required>
            </div>
            <div class="col-md-3 p-2">
              <input type="text" class="form-control " placeholder="請輸入事件" name="events" required>
            </div>
            <div class="col-md-3 p-2">
              <input type="text" class="form-control " placeholder="請輸入詳情" name="details" required>
            </div>
